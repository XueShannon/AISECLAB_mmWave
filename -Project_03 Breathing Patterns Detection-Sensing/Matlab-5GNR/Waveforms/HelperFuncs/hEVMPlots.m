...
% Copyright 2020-2024 The MathWorks, Inc.
    ...
    % Plot EVM statistics( RMS and peak) for each valid symbol, slot and subcarrier
    plotLabel = [" Symbol";" Slot";" Subcarrier"];

    % Loop for symbols, slots and subcarriers
    for pIdx = 1:3

        % Vector containing EVM statistics ( RMS or peak)
        if contains(plotLabel(pIdx),"Symbol")
            evmRMS = evmSymbolRMS;
            evmPeak = evmSymbolPeak;
        elseif contains(plotLabel(pIdx),"Slot")
            evmRMS = evmSlotRMS;
            evmPeak = evmSlotPeak;
        elseif contains(plotLabel(pIdx),"Subcarrier")
            evmRMS = evmSubcarrierRMS;
            evmPeak = evmSubcarrierPeak;
        end
        subplot(3,1,pIdx)
        plot(0:length(evmRMS)-1,evmRMS,'.-',0:length(evmPeak)-1,evmPeak,'.:');

        % Build a string with the intended label inserted at the
        % appropriate location
        str1 = ' EVM vs ';
        str2 = plotLabel(pIdx);
        str3 = strcat(str1,str2,' BWP index : ');
        title(strcat(label,str3,num2str(bwpIdx)));
        grid on; xlabel(str2); ylabel('EVM (%)'); legend('rms EVM','peak EVM','Location','bestoutside')
        yLim = max(evmPeak)*1.1;
        yTickStep = linspace(0,yLim,3);
        yticks(yTickStep)
        axis([0 length(evmRMS)-1 0 yLim])
    end
